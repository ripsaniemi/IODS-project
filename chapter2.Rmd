...
data <- read.csv("data/learning2014.cvs")
...
We see that the tree variables that have the highest (absolute) correlation with points are attitude, stra and surf. Lets start with a linear regression with those three as explanatory variables:
...
```
Our p-value is 3.125e-08, so this there is statistically significant relationship between explanotory variables with points. In this model alpha would be 11.02, beeta1 0.34, beeta2 0.85 and beeta3 -0.59.

However, When we look at these three variabes and their, only one of them, attitude, has a statistically significant p-value = 1.93e-08 (<0.05). For this reason, let's take stra and surf out of the regression, since we can conclude that they are not making this model any better.
...
This means that when attitude increases, points increases as well. This result can be seen through the scatterplot where the regression line is drawn as well.

R-squared is 0.19, which means that this model explains 19% of the variation of dependent variable Points.  


## 5)
## Diagnostic plots anfd graphical model validation
