...
## Reading the datasets and factoring some vars again
```{r}
...
```
We see that in all groups the weigth seems to increase when time increases. In groups 2 and 3 increase is more clear. Graph shows us that there seems to be a difference at least between Group 1 vs other Groups. Groups 2 ja 3 are more similar. In Group 2 there is one unsual individual, who we migth delete later.
...
```
The mean profiles plot shows that the error bars do not overlap, so it is pretty safe to say there is a difference between all groups.
...
Next we can test the group difference with t-test.
...
```
From t-tests we see that between Groups 1 and 2 and 2 and 3 the mean in weigths differ. Between group 1 and 3 there is no significant difference.  

This means that Group 2 diet is the one that had caused an significant growth in weigth.
...
The the p-value of week is very small, so week has an negative effect on bprs. This means both treatments seems to work, but not differently.
